...
def send_email(
    *,
    ...
    smtp_options = {
        ...
        if hasattr(settings, "SMTP_CERT_FILE")
        else None,
    }
...
def generate_password_reset_token(session: Session, email: str) -> str:
    ...
    # Securely store the token hash in the database to track usage
    token_hash = hashlib.sha256(encoded_jwt.encode()).hexdigest()
    db_token = PasswordResetToken(
        token_hash=token_hash, email=email, created_at=now, used=False
    )
